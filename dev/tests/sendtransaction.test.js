...
describe('send transaction', () => {
  ...
  test('one transaction authorizes exactly one dispatch invocation', () => {
    ...
    // Post-begin body must not contain a second actuator call.
    const begin = send.indexOf('_beginSendAttempt');
    expect(send.slice(begin)).not.toContain('.click()');
    expect(send.slice(begin)).not.toContain('requestSubmit');
  });
