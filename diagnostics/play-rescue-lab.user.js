...
function rescue(r){const t=r.querySelector('.g-mod-transport');if(!t||r.querySelector('#gitl-play-rescue'))return;const box=node('div');box.id='gitl-play-rescue';const h=node('div','pr-head');h.append(node('span','','Play rescue engines'),node('span','','TEST'));box.append(h,node('div','pr-sub','Primary stays default. If it fails, manually try one independent fallback. Never try another method after an uncertain send.'));const actions=node('div','pr-buttons'),a=btn('gitl-alpha','Alpha · semantic Send'),b=btn('gitl-beta','Beta · native form');actions.append(a,b);box.append(actions);const st=node('div','','Ready · Primary default · Alpha/Beta available for testing');st.id='gitl-play-rescue-status';st.setAttribute('aria-live','polite');box.append(st);const sug=node('div');sug.id='gitl-play-rescue-suggest';sug.hidden=true;box.append(sug);const fb=node('div');fb.id='gitl-play-rescue-feedback';fb.hidden=true;const yes=btn('gitl-worked','👍 Worked'),no=btn('gitl-failed','👎 Failed');fb.append(yes,no);box.append(fb);t.insertAdjacentElement('afterend',box);a.addEventListener('click',alpha);b.addEventListener('click',beta);yes.addEventListener('click',()=>record(true));no.addEventListener('click',()=>record(false));paint();}
