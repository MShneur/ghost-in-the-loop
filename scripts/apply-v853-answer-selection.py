...
anchor = "// Adapter — all DOM reads/writes\nconst Adapter = {"
helper = r'''/* Choose the actual latest assistant answer, not merely the last item
...
const Adapter = {'''
if anchor not in s:
    raise SystemExit('Adapter anchor not found')
...
if old not in s:
    raise SystemExit('getLastText block not found')
s = s.replace(old, new, 1)
p.write_text(s)

Path('tests/perplexity-answer-selection.test.js').write_text(r'''const fs = require('fs');
