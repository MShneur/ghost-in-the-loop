...
if (boundary.includes(oldDefault)) {
  boundary = boundary.replace(oldDefault, newDefault);
} else if (!boundary.includes(newDefault)) {
  throw new Error('basic boundary default anchor not found');
}
...
if (boundary.includes(oldCommittee)) {
  boundary = boundary.replace(oldCommittee, newCommittee);
} else if (!boundary.includes(newCommittee)) {
  throw new Error('basic boundary committee anchor not found');
}
