...
const EXPOSE = `
  window.__GITL_LongChatPerf = Object.freeze({
    ...
    beginSendProbe: () => {
      ...
      return _beginSendAttempt('long-chat-a2-read-only', Adapter.peekInput());
    },
    prepareSendEvidenceProbe: () => {
      GHOST.loop.sendTxn = {
        id: 'long-chat-a2', state: 'dispatching', path: 'benchmark-read-only', attemptedAt: Date.now(),
...
function historyHtml(turns) {
  ...
  for (let i = 0; i < turns; i += 1) {
    ...
    else if (i === turns - 2) inner = `NEWEST_UNFINISHED_${turns}: this is the newest visible unfinished assistant response and intentionally contains no terminal marker so stale older HALT or PROCEED markers cannot win selection.`;
    else if (i === turns - 1) { inner = `HIDDEN_DECOY_${turns}: hidden duplicate after the newest visible response. [[GITL::PROCEED]]`; style = ' style="display:none" aria-hidden="true"'; }
...
async function loadFixture(page, turns) {
  ...
  await page.goto(`https://chatgpt.com/c/gitl-long-chat-a2-${turns}`);
...
async function measure(page, operation, samples) {
  return page.evaluate(({ operation, samples }) => {
    ...
    return { times, query: window.__GITL_LongChatQueries.snapshot(), events: { ...events } };
  }, { operation, samples });
}

test.describe('R5 A2 grouped-selector tail collector', () => {
  test('falsifies the selected optimization against the A1 exact baseline', async ({ page }, testInfo) => {
    ...
    for (const turns of sizes) {
      ...
      for (const op of [answer, beginSend, sendEvidence]) expect(op.events).toEqual({ submit: 0, click: 0, input: 0, keydown: 0 });
      rows.push({
        turns,
        domNodes: correctness.domNodes,
        answerSelection: { p50Ms: percentile(answer.times, 50), p95Ms: percentile(answer.times, 95), qsaCallsPerSample: answer.query.calls / samples, qsaMatchesPerSample: answer.query.totalMatches / samples, qsaMsPerSample: answer.query.totalMs / samples, bySelector: answer.query.bySelector },
        beginSendObservation: { p50Ms: percentile(beginSend.times, 50), p95Ms: percentile(beginSend.times, 95), qsaCallsPerSample: beginSend.query.calls / samples, qsaMatchesPerSample: beginSend.query.totalMatches / samples },
        sendEvidenceObservation: { p50Ms: percentile(sendEvidence.times, 50), p95Ms: percentile(sendEvidence.times, 95), qsaCallsPerSample: sendEvidence.query.calls / samples, qsaMatchesPerSample: sendEvidence.query.totalMatches / samples }
      });
    }
